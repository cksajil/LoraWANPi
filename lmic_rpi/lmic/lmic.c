...
#define DNW2_SAFETY_ZONE ms2osticks(1000)
#endif
...
#define maxFrameLen(dr) ((dr) <= DR_SF9 ? maxFrameLens[(dr)] : 0xFF)
const u1_t maxFrameLens[] = {64, 64, 64, 123, 250, 250, 250};
...
static const s1_t TXPOWLEVELS[] = {
    30, 27, 24, 21, 18, 14, 10, 0, 0, 0, 0, 0, 0, 0, 0, 0};
#define pow2dBm(mcmd_ladr_p1) (TXPOWLEVELS[(mcmd_ladr_p1 & MCMD_LADR_POW_MASK) >> MCMD_LADR_POW_SHIFT])
...
#define dr2hsym(dr) (DR2HSYM_osticks[(dr)])
    ...
    us2osticksRound(128 << 3), // DR_SF8
    us2osticksRound(128 << 2), // DR_SF7
    us2osticksRound(128 << 1)  // DR_SF7B (if applicable)
#endif
};
...
static ostime_t nextJoinState(void)
{
    ...
    {
        LMIC.txChnl = (LMIC.txChnl + 1) % 3;
        setDrJoin(DRCHG_NOJACC, decDR((dr_t)LMIC.datarate));
    }

    LMIC.txend = os_getTime() +
                 (isTESTMODE()
                      ? dn2osticks(TESTMODE_JOIN_GUARD_ms)
                  : LMIC.datarate != DR_SF12
                      ? sec2osticks(1)
                      : sec2osticks(3 + LMIC.txChnl));
    return failed;
}
...
static void updateTx(ostime_t txbeg)
{
    ...
    LMIC.txpow = band->txpow;
    band->avail = txbeg + airtime * band->txcap;
    printf("%u: freq=%u\n", (unsigned int)os_getTime(), (unsigned int)LMIC.freq);
    ...
    while (oidx < olen)
    {
        ...
        case MCMD_LADR_REQ:
        {
            ...
            if (!validDR(dr))
            {
                ...
                EV(specCond, ERR, (e_.reason = EV::specCond_t::BAD_MAC_CMD, e_.eui = MAIN::CDEV->getEui(), e_.info = Base::lsbf4(&d[pend]), e_.info2 = Base::msbf4(&opts[oidx - 4])));
            }
        ...
        case MCMD_BCNI_ANS:
        {
            ...
            if ((LMIC.opmode & OP_TRACK) == 0)
            {
                ...
                // Setup RX parameters
                LMIC.bcninfo.txtime = (LMIC.rxtime + ms2osticks(os_rlsbf2(&opts[oidx + 1]) * MCMD_BCNI_TUNIT) + ms2osticksCeil(MCMD_BCNI_TUNIT / 2) - BCN_INTV_osticks);
                ...
                EV(lostFrame, INFO, (e_.reason = EV::lostFrame_t::MCMD_BCNI_ANS, e_.eui = MAIN::CDEV->getEui(), e_.lostmic = Base::lsbf4(&d[pend]), e_.info = (LMIC.missedBcns | (osticks2us(LMIC.bcninfo.txtime + BCN_INTV_osticks - LMIC.bcnRxtime) << 8)), e_.time = MAIN::CDEV->ostime2ustime(LMIC.bcninfo.txtime + BCN_INTV_osticks)));
            }
            oidx += 4;
            continue;
        }
        }
        EV(specCond, ERR, (e_.reason = EV::specCond_t::BAD_MAC_CMD, e_.eui = MAIN::CDEV->getEui(), e_.info = Base::lsbf4(&d[pend]), e_.info2 = Base::msbf4(&opts[oidx])));
        break;
    }
    if (oidx != olen)
    {
...
static bit_t processJoinAccept(void)
{
    ...
    nojoinframe:
        ...
        EV(devCond, DEBUG, (e_.reason = EV::devCond_t::NO_JACC, e_.eui = MAIN::CDEV->getEui(), e_.info = LMIC.datarate | DR_PAGE, e_.info2 = osticks2ms(delay)));
...
static void buildDataFrame(void)
{
    ...
    LMIC.frame[OFF_DAT_FCT] = (LMIC.dnConf | LMIC.adrEnabled | (LMIC.adrAckReq >= 0 ? FCT_ADRARQ : 0) | (end - OFF_DAT_OPTS));
    ...
    {
        EV(devCond, INFO, (e_.reason = EV::devCond_t::RE_TX, e_.eui = MAIN::CDEV->getEui(), e_.info = LMIC.seqnoUp - 1, e_.info2 = ((LMIC.txCnt + 1) | (DRADJUST[LMIC.txCnt + 1] << 8) | ((LMIC.datarate | DR_PAGE) << 16))));
    }
    ...
    if (txdata)
    {
        ...
        if (LMIC.pendTxPort != 223)
        { // port 223 unencrypted for testing (TT)
